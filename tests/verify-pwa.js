...
(async () => {
  ...
  try {
    ...
    ok(await page.locator('#cat-tiles .cat-tile').count() === 7, 'app boots over http with all 7 shelves');
    ...
    ok(await page.locator('#cat-tiles .cat-tile').count() === 7, 'OFFLINE reload still boots the full app');
