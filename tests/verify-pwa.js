...
(async () => {
  ...
  try {
    ...
    ok(await page.locator('#home-grid .game-card').count() === 25, 'app boots over http with 25 cards');
    ...
    ok(await page.locator('#home-grid .game-card').count() === 25, 'OFFLINE reload still boots the full app');
