...
(async () => {
  ...
  try {
    ...
    ok(await page.locator('#home-grid .game-card').count() === 26, 'app boots over http with 26 cards');
    ...
    ok(await page.locator('#home-grid .game-card').count() === 26, 'OFFLINE reload still boots the full app');
